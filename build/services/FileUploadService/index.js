"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const fileConfig_1 = __importDefault(require("../../config/fileConfig"));
const helpers_1 = __importDefault(require("../../helpers"));
// import AwsStorageProvider from './AwsStorageProvider';
const CloudinaryStorageProvider_1 = __importDefault(require("./CloudinaryStorageProvider"));
const LocalDiskStorageProvider_1 = __importDefault(require("./LocalDiskStorageProvider"));
class FileUploadService {
    constructor() {
        this.fileSystemProvider =
            fileConfig_1.default.default === 'cloudinary'
                ? new CloudinaryStorageProvider_1.default()
                : // : fileConfig.default === 's3'
                    // ? new AwsStorageProvider()
                    new LocalDiskStorageProvider_1.default();
    }
    uploadFile(file, resource_type, folder) {
        return __awaiter(this, void 0, void 0, function* () {
            const filename = helpers_1.default.generateUniqueFilename(file.originalname);
            return this.fileSystemProvider.uploadFile(file, resource_type, folder);
        });
    }
    deleteFile(filename) {
        return __awaiter(this, void 0, void 0, function* () {
            // Delete the file using the storage provider
            // await this.storageProvider.deleteFile(filename);
        });
    }
}
exports.default = FileUploadService;
